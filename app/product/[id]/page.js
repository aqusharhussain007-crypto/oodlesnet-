...
export default function ProductPage({ params }) {
  ...
  return (
    <div style={{ padding: 16, paddingBottom: 96, maxWidth: 720, margin: "0 auto" }}>
      ...
      {/* IMAGE */}
      <div style={{ borderRadius: 18, overflow: "hidden", background: "#fff", boxShadow: "0 6px 18px rgba(0,0,0,0.1)", marginBottom: 16 }}>
        <Image
      ...
      <div style={{ display: "flex", gap: 16, overflowX: "auto", padding: "16px 0" }}>
        {sortedStores.map((store, index) => (
          <div key={index} style={{ minWidth: 260, background: "#fff", padding: 20, borderRadius: 18, boxShadow: "0 6px 14px rgba(0,0,0,0.1)", border: "1px solid #e5e7eb", flexShrink: 0 }}>
            <div style={{ fontSize: 18, fontWeight: 800 }}>
              {store.name}
            </div>

            {typeof store.price === "number" && (
              <div style={{ fontSize: 22, fontWeight: 900, margin: "8px 0", color: store.price === cheapest ? "#16a34a" : "#2563eb" }}>
